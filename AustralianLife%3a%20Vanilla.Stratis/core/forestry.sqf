...
[
	...
	["corn_seed", "BUSH", "corn_stock", "AS_p_wheat_EP1", 30, ["corn_seed", 5, "", "normal"], 35, "Corn Stock", ["forestry", 1, 1240]],
	
	//TIER 2
	["potato_sprouts", "BUSH", "potato_patch", "MAP_c_fern", 20, ["patato_sprouts", 5, "", "normal"], 35, "Pumpkin Patch", ["forestry", 10, 820]],
	["alder_seed", "TREE", "alder_tree", "MAP_t_pinusN2s", 10, ["alder_seed", 8, "sickle", "normal"], 20, "Alder Tree", ["forestry", 10, 980]],
	
	//TIER 3
	
	["rubber_seed", "TREE", "rubber_tree", "MAP_t_pinusN2s", 10, ["rubber_seed", 8, "sickle", "normal"], 20, "Rubber Tree", ["forestry", 20, 980]],
	["pumpkin_seed", "BUSH", "pumpkin_patch", "MAP_pumpkin", 20, ["pumpkin_seed", 5, "", "normal"], 35, "Pumpkin Patch", ["forestry", 15, 1240]],
	["pine_seed", "TREE", "pine_tree", "MAP_t_picea2s", 30, ["pine_seed", 12, "sickle", "normal"], 20, "Pine Tree", ["forestry", 20, 1650]],
	["apple_seed", "BUSH", "apple_bush", "MAP_t_betula1f", 20, ["apple_seed", 5, "sickle", "normal"], 35, "Apple Tree", ["forestry", 25, 2250]],
...
RPP_var_forestry_typeBushes = [ "as_p_fiberPlant_EP1", "MAP_pumpkin2", "MAP_c_fern", "MAP_t_betula1f", "MAP_pumpkin", "AS_p_wheat_EP1" ];
...
RPP_fnc_forestry_plantSprout = 
{
    ...
    if (count _nearestObjects <= 0) then
    {
        ...
        {
            localize "STRS_forestry_onRoad" call RPP_fnc_hint;
        };
    }
    else
    {
...
RPP_fnc_forestry_setupTree = 
{
    ...
    _object setVariable["burning", false, true];
        
        _text = format["<t size='0.55' color='#008000'>%1</t><br/><t size='0.4'>(Press F to cutdown)<br/></t><t size='0.4'>(Press T to pick sprouts)<br/></t><t size='0.4'>(Press Ctrl+B to burn the plant)<br/></t>", _plantName];
...
RPP_fnc_forestry_setupBush = 
{
	...
	_object setVariable["numResources", 0, true];
        ...
        _text = format["<t size='0.55' color='#008000'>%1</t><br/><t size='0.4'>(Press F to harvest)<br/></t><t size='0.4'>(Press T to pick sprouts)<br/></t><t size='0.4'>(Press B to check)<br/></t><t size='0.4'>(Press Ctrl+B to burn the plant)<br/></t>", _plantName];
...
RPP_fnc_forestry_burnPlant =
{
	private ["_object","_burnIntensity","_smokeFX"];
    
    _object = _this select 0;
    
    if( _object getVariable "burning") exitWith
    {
        //already burning. no need to tell the player that it's already burning.
    };
    _object setVariable ["burning", true, true];
    _burnIntensity = 9.5;//in fu ture we will add on to tree array to configure burn intensity of each plant
    _smokeFX = [_object, 9.5, time, false, false] spawn BIS_Effects_Burn;
