...
[
	...
	[item_factory_storage, "Lost Property Storage", 0, false],
	[manufacturing_plant_storage,"Manufacturing Plant Storage", 0, false],
	[car_factory_storage, "Vehicle Factory Storage", 0, false],
...
RPP_fnc_trunkTake = 
{
    ...
    if (_type == "WEAPON") then
    {
        ...
        else
        {
            ...
            {
                _itemArr = ["ItemRadio","ItemWatch","ItemMap","ItemGPS","ItemCompass","NVGoggles","Binoculars"];
                {
	                if((not ([player,_x] call RPP_fnc_hasWeaponClass))&& _itemClass == _x) then
	                {
	                    _ok = true;
	                };
                }
