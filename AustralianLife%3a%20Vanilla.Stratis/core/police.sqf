...
RPP_fnc_pol_paidTicket = {
	...
	if (_success) then {
		...
		if(player == _cop) then
		{
			...
			{
				_bounty = floor (_amount * 0.6);
			};
		}
		else
		{
			_numCops = _numCops - 1;
			_bounty = floor ( _amount * 0.4 / _numCops );
		};
		RPP_var_bankAmount = RPP_var_bankAmount + _bounty;
        [RPP_var_saving_tableBanks, ["RPP_var_bankAmount", RPP_var_acc_login], RPP_var_bankAmount] call RPP_fsav_clientRequestSave;
		format[localize "STRS_ticket_copPaid", name _civ, _amount, _bounty] call RPP_fnc_hint;
	} else {
...
RPP_fnc_pol_payTicket = {
	...
	["RPP_fnc_pol_paidTicket", [_success, _from, _to, _amount]] call RPP_fnet_execPublic;
